...
export function KeyboardShortcutsHelp() {
	...
	return (
		<div className="relative group">
			<HelpCircle className="w-4 h-4 text-slate-500 hover:text-[#7C5CFF] transition-colors cursor-help" />
			...
			<div className="absolute right-0 top-full mt-2 w-64 bg-[#09090b] border border-white/10 rounded-lg p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-xl z-50">
				<div className="flex items-center justify-between mb-2">
					...
					<button
						...
						className="flex items-center gap-1 text-[10px] text-slate-500 hover:text-[#7C5CFF] transition-colors"
					>
					...
					{SHORTCUT_ACTIONS.filter((action) => BLUR_REGIONS_ENABLED || action !== "addBlur").map(
						(action) => (
							<div key={action} className="flex items-center justify-between">
								...
								<kbd className="px-1 py-0.5 bg-white/5 border border-white/10 rounded text-[#7C5CFF] font-mono">
