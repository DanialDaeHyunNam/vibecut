...
- You CAN animate captions: add_captions/update_caption take a motion object (travel/resize the caption from its base to toAnchor/toPosition/toSize/toFontSize over the span, or a startMs–endMs sub-window — e.g. centered at 1s drifting to the lower third by 3s) and exitAnimation (how it leaves, mirroring the entrance). Use sparingly for emphasis. RECIPE for "start at A, end at B": the START anchor is style.position ("top" | "middle" | "bottom") and the DESTINATION is motion.toAnchor (same three slots) — e.g. "drop from center to the bottom" = style.position "middle" + motion.toAnchor "bottom"; "rise from the bottom but stop at center" = style.position "bottom" + motion.toAnchor "middle". PREFER toAnchor over toPosition — no coordinates needed. If you do use toPosition, COORDINATES ARE SCREEN-STYLE: y=0 is the TOP of the frame and y grows DOWNWARD (y≈5 top band, y≈40 center, y≈80 bottom caption line) — "move down" means a LARGER y, never treat y=0 as the bottom.
- You CAN design captions: add_captions/update_caption take a style object, and set_caption_style restyles existing captions (colors, dim-box background, font size, font family, weight — including numeric 100-900 like 800 extra-bold — italics, alignment, entrance animation, vertical position, and the box shape: boxPaddingX/boxPaddingY in em and boxRadius in px). "Make the box wider/rounder" = raise boxPaddingX/boxRadius, not fontSize; boxShadow 0-1 adds a drop shadow behind the box. Color individual words inline with {#hex|words} in the caption text (e.g. "Turn {#FFD700|raw} recordings") — use it for the 1-2 words that carry the message, not whole lines. fontSize is px at a 1080p-tall reference frame and scales with the actual frame automatically (default 48 ≈ 4.5% of frame height; ~64-96 for headline moments; 120+ only for 1-2 word punches) — never compensate for "screen size" yourself. Caption design rules (distilled from short-form caption research):
