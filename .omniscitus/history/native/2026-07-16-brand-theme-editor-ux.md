...
### 2026-07-20 (밤 연속 세션)
**Focus**: 구간 선택 두-핸들 UX 완성(피드백 6회전) + Shift+↑/↓ 레인 전환 + 팁 캐러셀 자동 순환
- **두-핸들 UX**: PlaybackControls shift+드래그 전면 제거(239→121줄). 플레이헤드 "+"(눈금자 row 오른쪽 10px·세로 중앙 — 다이아몬드와 충돌 3회 이터레이션 끝 확정) → 두 핸들 밴드 생성(시작=플레이헤드, 길이 ~10%/1–10s) → 드래그 시 **블록 시작/끝·0·영상 끝·플레이헤드에 스냅**(px 14 기준을 pixelsToValue로 ms 환산 — 줌 레벨 무관 동일 손맛) → "@ Add to chat 필"로 주입. 필은 오버레이가 아니라 **마지막 레인 뒤 normal flow 스트립(mt-1)** — 절대배치 bottom이 계속 어긋나던 문제의 구조적 해법. 색은 플레이헤드 바이올렛과 구분되는 **에메랄드 그린** 계열로 분리
- **Shift+↑/↓ 레인 포커스 전환**: Alt+←/→가 쓰는 activeLane을 시각 순서(줌→트림→자막→배속)로 순환, 입력/슬라이더 포커스 시 무시. FIXED_SHORTCUTS laneSwitch(충돌 검사 자동 포함)+13로케일
- **팁 캐러셀**: 신기능 팁 3개 추가(⇧+↑/↓, "+ → @" 구간 선택, "@" 블록 배지 — 5→8개) + **자동 순환**(9s 인터벌, 호버 일시정지, ‹ › 수동 조작 시 세션 동안 영구 정지)
- i18n 신규 키 21개 13로케일 + 전수 점검(키 실재 273건·패리티·하드코딩 스윕 — 잔여 하드코딩 aria 8건은 전부 기존 것)
